...
3. **Linux builds a Flatpak, but `briefcase create` still needs host GTK dev libraries.** `briefcase package` would prompt to choose a format on Linux, so pass `-p flatpak`. The runner needs `flatpak` + `flatpak-builder` (for packaging) **and** the GTK dev headers (`gir1.2-gtk-3.0 libcairo2-dev libcanberra-gtk3-module libgirepository1.0-dev`) — Briefcase's Linux `create` step verifies these on the host *regardless of the eventual format*, even though the Flatpak build itself runs inside the `org.gnome.Sdk` sandbox. Briefcase then registers the Flathub remote and pulls the runtime itself. (Flatpak cannot be built inside a Docker container — `ubuntu-latest` runs jobs on the bare VM, so this is fine.)
